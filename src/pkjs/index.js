...
function syncSettingsFromWatch(payload) {
    ...
    var syncColor = function(payloadKey, settingsKey) {
        syncSetting(payloadKey, settingsKey, function(v) {
            // Treat as unsigned 32-bit int and format as hex string
            var hex = (v >>> 0).toString(16).toUpperCase();
            while (hex.length < 6) hex = '0' + hex;
            // Handle cases where the alpha channel might be included (e.g. 0xFFRRGGBB)
    ...
    if (updated) {
        console.log('Synchronized Clay settings from watch: ' + JSON.stringify(settings));
...
Pebble.addEventListener('appmessage', function(e) {
    ...
    // 1. Always sync settings first
    syncSettingsFromWatch(payload);

    var hasForecastData = Boolean(payload.WATCH_HAS_FORECAST_DATA);
    if (hasForecastData) {
        // 2. Sync fetch time if we have data
        syncFetchTimeFromWatch(payload);
        ...
        if (lastFetchSuccess && lastFetchSuccess.time !== null) {
            if (Date.now() - new Date(lastFetchSuccess.time).getTime() > 1000 * 60 * 10) {
                ...
                console.log('Forecast data is fresh, skipping startup fetch.');
                app.pendingStartupFetch = false;
                return;
            }
        }
